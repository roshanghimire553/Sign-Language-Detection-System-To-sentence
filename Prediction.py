...
class ModelPredictor:
    ...
    def predict(self, test_image):
        ...
        # Check if the current gesture is "next" and the previous gesture is not "next"
        if ch1 == "next" and self.prev_char != "next":
            # Check if the gesture two counts back is not "next"
            if self.ten_prev_char[(self.count - 2) % 10] != "next":
                # If the gesture two counts back was "Backspace", remove the last character
                if self.ten_prev_char[(self.count - 2) % 10] == "Backspace":
                    self.str = self.str[:-1]  # Remove the last character
                else:
                    # Otherwise, add the previous gesture to the string if it wasn't "Backspace"
                    if self.ten_prev_char[(self.count - 2) % 10] != "Backspace":
                        self.str += self.ten_prev_char[(self.count - 2) % 10]  # Append the gesture

            else:  # If the gesture two counts back was "next"
                # Check if the last gesture was not "Backspace"
                if self.ten_prev_char[(self.count - 0) % 10] != "Backspace":
                    self.str += self.ten_prev_char[(self.count - 0) % 10]  # Append the last gesture

        # Check if the current gesture is a double space and the previous gesture is not
        if ch1 == "  " and self.prev_char != "  ":
            self.str += "  "  # Add two spaces to the string

        # Update the previous character to the current one
        self.prev_char = ch1
        self.current_symbol = ch1  # Store the current gesture
        self.count += 1  # Increment the gesture count
        self.ten_prev_char[self.count % 10] = ch1  # Update the history with the current gesture
